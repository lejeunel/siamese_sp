...
class Residual_Covolution(nn.Module):
    def __init__(self, icol, ocol, num_classes):
        ...
        self.conv2 = nn.Conv2d(ocol,
                               ...
                               dilation=12,
                               bias=True)
        self.conv3 = nn.Conv2d(num_classes,
                               ocol,
                               kernel_size=1,
                               stride=1,
                               padding=0,
...
class MS_Deeplab(nn.Module):
    ...
    def forward(self, x):
        ...
        self.interp1 = nn.Upsample(size=(int(input_size * 0.75) + 1,
                                         int(input_size * 0.75) + 1),
                                   mode='bilinear')
        self.interp2 = nn.Upsample(size=(int(input_size * 0.5) + 1,
                                         int(input_size * 0.5) + 1),
                                   mode='bilinear')
        self.interp3 = nn.Upsample(size=(output_size, output_size),
                                   mode='bilinear')
